...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || $price <= 0 || empty($description)) {
        ...
        $uploadDirPath = __DIR__ . '/../uploads/product_images/'; // absolute filesystem path
        ...
        $targetFilePath = $uploadDirPath . $imageName;
        $allowedTypes = ['jpg', 'jpeg', 'png', 'gif'];
        $fileType = strtolower(pathinfo($imageName, PATHINFO_EXTENSION));

        if (!$error && !in_array($fileType, $allowedTypes)) {
            $error = "Only JPG, JPEG, PNG, GIF files are allowed.";
        } elseif (!$error && move_uploaded_file($_FILES['image']['tmp_name'], $targetFilePath)) {
            // Insert into database (store filename only)
